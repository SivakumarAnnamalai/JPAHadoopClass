...
public class WCReducerUsingDistributedCache extends Reducer<Text,LongWritable,Text,LongWritable> {
    ...
    public void setup(Context ctx) throws IOException {
        ...
        for(Path path:paths){
            ctx.getCounter("Cache Files",path.toString()).increment(1);
        ...
        ctx.getCounter("Distributed Cache FileLocation",cacheFileLocation).increment(1);
        ctx.getCounter("Distributed Cache Value of Hadoop",properties.getProperty("hadoop")).increment(1);
    }
